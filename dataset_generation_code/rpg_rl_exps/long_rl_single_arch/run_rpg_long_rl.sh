...
WORK_DIR="${WORK_DIR:-/work/ADS_shared/dataset_generation_code/rpg_rl_exps/long_rl_single_arch}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/long_rl_single_arch_cache}"
export WANDB_DIR="${WANDB_DIR:-$WORK_DIR/wandb}"
export WANDB_DATA_DIR="${WANDB_DATA_DIR:-$WORK_DIR/wandb-data}"
export WANDB_CACHE_DIR="${WANDB_CACHE_DIR:-/tmp/long_rl_single_arch_wandb_cache}"
export TRITON_CACHE_DIR="${TRITON_CACHE_DIR:-/tmp/long_rl_single_arch_triton_cache}"
export TILELANG_CACHE_DIR="${TILELANG_CACHE_DIR:-/tmp/long_rl_single_arch_tilelang_cache}"
export FLASHINFER_WORKSPACE_BASE="${FLASHINFER_WORKSPACE_BASE:-/tmp/long_rl_single_arch_flashinfer}"
export UV_CACHE_DIR="${UV_CACHE_DIR:-/tmp/long_rl_single_arch_project_uv_cache}"
...
LOGGER="${LOGGER:-wandb}"
CKPT_DIR="${CKPT_DIR:-/data/long_rl_single_arch/checkpoints}"
...
uv run --isolated --extra fsdp -m long_rl_single_arch.main_rpg_long \
  ...
  trainer.project_name="rpg_long_rl_single_arch" \
  trainer.run_name="rpg_qwen3.5_9b_grpo_lora_dose_window_original_reward" \
