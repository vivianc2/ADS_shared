...
# ---------------------------------------------------------------------------
# LLM wrappers (lightweight, no dependency on scientist_agent)
# ---------------------------------------------------------------------------
...
def load_worlds(worlds_dir: Optional[str] = None,
                world_json: Optional[str] = None) -> List[Dict[str, Any]]:
...
def run_zero_shot(worlds: List, llm: ZeroShotLLM) -> List[Dict[str, Any]]:
    """Run zero-shot baseline on all worlds/questions. Returns list of result dicts."""
    all_results = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Zero-shot baseline for causal discovery questions",
    ...
    if args.backend == "openai":
        ...
        if not args.model:
            logger.error("--model is required for bedrock backend (e.g. us.meta.llama3-3-70b-instruct-v1:0)")
            sys.exit(1)
    ...
    if args.output:
        ...
        base = f"./results/zero_shot_{timestamp}"
